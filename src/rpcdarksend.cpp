...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <EnergyCoinaddress> <amount>\n"
            "EnergyCoinaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <EnergyCoinaddress> <amount>\n"
            "EnergyCoinaddress, denominate, or auto (AutoDenominate)"
            "<amount> is type \"real\" and will be rounded to the nearest 0.1"
            + HelpRequiringPassphrase());

    CEnergyCoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in EnergyCoin.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in EnergyCoin.conf\n"
                ...
                "  vote-many    - Vote on a EnergyCoin initiative\n"
                "  vote         - Vote on a EnergyCoin initiative\n"
                );
    ...
    if (strCommand == "current")
    {
        ...
        if(winner) {
            ...
            CEnergyCoinAddress address2(address1);
    ...
    if (strCommand == "genkey")
    {
        ...
        return CEnergyCoinSecret(secret).ToString();
    }
